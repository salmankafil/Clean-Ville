            ...
            <div class="event-details">
                ...
                @if(auth()->check())
                    ...
                    @else
                        ...
                        @else
                             <button type="button" class="cta-button volunteer-btn" data-bs-toggle="modal" data-bs-target="#DateModal" data-event-id="{{ $event->id }}">Volunteer</button>
...
<div class="modal fade" id="DateModal" tabindex="-1" aria-labelledby="DateModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="DateModalLabel">Select Preferred Date:</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            var volunteerButtons = document.querySelectorAll('.volunteer-btn');
            var modal = new bootstrap.Modal(document.getElementById('DateModal'));

            volunteerButtons.forEach(function (button) {
                button.addEventListener('click', function () {
                    var eventId = button.getAttribute('data-event-id');
                    fetchDateRange(eventId);
                    modal.show();
                });
            });

        var modalBody = document.querySelector('#DateModal .modal-body');
     function fetchDateRange(eventId) {
    fetch(`/get-date-range/${eventId}`)
        .then(response => response.json())
        .then(data => {
            // Update the modal content or perform other actions with the date range
        ...
        function updateModalBody(dateRange) {
        ...
        if (dateRange.length > 0) {
            dateRange.forEach(function (date) {
                ...
                input.setAttribute('name', 'preferredDate');
                input.setAttribute('value', date);

                var label = document.createElement('label');
                label.classList.add('form-check-label');
                label.setAttribute('for', 'preferredDate');

                // You may want to format the date here, depending on your requirements
                label.textContent = date;

                formCheck.appendChild(input);
                formCheck.appendChild(label);

                // Append the formCheck to the modal body
                modalBody.appendChild(formCheck);
            });
        } else {
            // Display a message if no date range is available
            var noDateRangeMessage = document.createElement('p');
            noDateRangeMessage.textContent = 'No date range available for this event.';
            modalBody.appendChild(noDateRangeMessage);
        }
    }
